...
func (s *MysqlComentarioStore) ListarPublicos(publicacion_id string) ([]models.Comentario, error) {
	...
	var err = s.db.Select(&comentarios, `SELECT id, COALESCE(padre_id, "") as padre_id, nombre, es_autor, autor_original, contenido, COALESCE(fecha_moderacion, "") as fecha_moderacion FROM comentarios WHERE estado="aprobado" AND publicacion_id=?`, publicacion_id)
	if err != nil {
